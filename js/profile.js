...
function initProfileContent(){
    document.querySelector(`#facebook-pic-id`).src = grabedFacebookData.userPicture_fb;
    document.querySelector(`#facebook-name-id`).innerHTML = grabedFacebookData.userName_fb;
    document.querySelector(`#facebook-email-id`).innerHTML = grabedFacebookData.userEmail_fb;
}
...
function logoutFB(){
        FB.getLoginStatus(function(response) {
            if (response.status === 'connected') {   // Logged into your webpage and Facebook.
                grabedFacebookData = {};
                localStorage.setItem('userFacebook', JSON.stringify(grabedFacebookData));
                alert('您已從Facebook登出');
                FB.logout();
...
init = () => {
    ...
    if(localStorage.getItem('userFacebook')){
        grabedFacebookData = JSON.parse(localStorage.getItem('userFacebook'));
    }
    //Init storage
    ...
    //document.querySelector('#facebook-logout-id').addEventListener('click', logoutFB);
}
